...
describe('resolveStoryRef', () => {
  ...
  it('errors when story code is ambiguous across products', async () => {
    P.story.findFirst.mockResolvedValueOnce(null)                              // id miss
    P.story.findMany.mockResolvedValueOnce([{ id: 'a' }, { id: 'b' }])        // ambiguous
    const r = await resolveStoryRef('ST-42', 'u1')
    expect('error' in r && /ambiguous/.test(r.error)).toBe(true)
  })
...
describe('resolveTaskRef', () => {
  it('returns the id when given a valid CUID', async () => {
    P.task.findFirst.mockResolvedValueOnce({ id: 'cmp_task_1' })
    expect(await resolveTaskRef('cmp_task_1', 'u1')).toEqual({ id: 'cmp_task_1' })
  })

  it('resolves a task code to its id', async () => {
    P.task.findFirst.mockResolvedValueOnce(null)                        // id miss
    P.task.findMany.mockResolvedValueOnce([{ id: 'cmp_task_2' }])      // code hit
    expect(await resolveTaskRef('T-99', 'u1')).toEqual({ id: 'cmp_task_2' })
  })

  it('hints when a story code is passed where a task is expected', async () => {
    P.task.findFirst.mockResolvedValueOnce(null)                        // id miss
    P.task.findMany.mockResolvedValueOnce([])                           // code miss
    P.story.findFirst.mockResolvedValueOnce({ id: 'cmp_story_9' })     // it's a story code
    const r = await resolveTaskRef('ST-1', 'u1')
    expect('error' in r && /story code/.test(r.error)).toBe(true)
  })
})
