...
it('rejects historical unbound jobs associated with an occupied Task directly or through sprint execution',()=>{
 expect(()=>assertUnmanagedJob({kind:'TASK_IMPLEMENTATION',task:{dispatch_request_id:'host-request'}})).toThrow('DISPATCH_MANAGED_ROW')
 expect(()=>assertUnmanagedJob({kind:'SPRINT_IMPLEMENTATION',task_executions:[{task:{dispatch_request_id:'host-request'}}]})).toThrow('DISPATCH_MANAGED_ROW')
})
