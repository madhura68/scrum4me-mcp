...
export async function codeAttempt(h:Awaited<ReturnType<typeof makeDispatchHarness>>,root:string,options:{verifyOnly?:boolean;change?:boolean;free?:boolean;staleHeartbeat?:boolean;corruptBundle?:boolean}={}){
 const verifyOnly=options.verifyOnly??false
  ...
  const attempts=createDispatchAttempts({...opts,credentialKeys:{1:Buffer.alloc(32,8)},keyVersion:1,startPermitPrivateKey:generateKeyPairSync('ed25519').privateKey,startPermitKeyId:'permit-test'}),claim=await attempts.claimDispatchAttempt(f.actor,session.incarnation_id,'task-claim',session.session_credential);if(!claim?.context)throw Error('claim required');const proof=claim.context.proof,scope={scopeId:randomUUID(),bootId:'boot',imageDigest,profileSha256};await attempts.startDispatchAttempt(f.actor,proof,scope)
  ...
  let bytes=await createCodeArtifact(repo,{repoUrl:repo,baseSha,headSha,branch,checks:result.checks})
  // Leave every field the completion transaction checks intact and corrupt only the bundle, so the
  // artifact is accepted and verification fails inside git instead of at an input check.
  if(options.corruptBundle){const d=JSON.parse(Buffer.from(bytes).toString('utf8'));d.bundle=Buffer.from('this is not a git bundle').toString('base64');bytes=Buffer.from(JSON.stringify(d),'utf8')}
