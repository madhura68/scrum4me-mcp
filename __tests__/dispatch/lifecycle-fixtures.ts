...
export async function running(h:DispatchHarness,options:{prepared?:boolean;review?:boolean;readSource?:boolean}={}){
 ...
 const attempts=createDispatchAttempts({...opts,credentialKeys:{1:Buffer.alloc(32,8)},keyVersion:1,startPermitPrivateKey:generateKeyPairSync('ed25519').privateKey,startPermitKeyId:'permit-test'}),receipt=await attempts.claimDispatchAttempt(f.actor,session.incarnation_id,'claim',session.session_credential)
