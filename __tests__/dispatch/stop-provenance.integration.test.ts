...
it('R23 stages a stopped prepared scope without registering it, accepting it, or freeing its reservation',async()=>{
 ...
 try{
  ...
  const claimed=await createDispatchAttempts({...opts,credentialKeys:{1:Buffer.alloc(32,8)},keyVersion:1,startPermitPrivateKey:generateKeyPairSync('ed25519').privateKey}).claimDispatchAttempt(f.actor,session.incarnation_id,'prepared-claim',session.session_credential)
