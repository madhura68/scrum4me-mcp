 ...
 * Pass attempts to process expired attempts as well as reservations. */
export function createDispatchTick(deps: { store: DispatchStore; selection: DispatchSelection; attempts?: DispatchAttempts }) {
  return async function dispatchTick(): Promise<{ reserved: number; retired: number; uncertain: number }> {
    ...
    return { reserved, retired, uncertain: await deps.attempts?.markExpiredAttempts() ?? 0 }
  }
}
