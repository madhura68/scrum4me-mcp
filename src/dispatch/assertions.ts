...
export function signDispatchAssertion(input: AssertionRequest & {
  issuer: AssertionIssuer; userId: string; jti: string; key: Buffer; now?: number
}): string {
...
export function verifyDispatchAssertion(input: AssertionRequest & {
  assertion: string; keys: DispatchAssertionKeys; now?: number
  ...
  if (!claims || typeof claims !== 'object' || Array.isArray(claims)
    || Object.keys(claims).sort().join(',') !== 'aud,body_sha256,exp,iat,iss,jti,method,path,sub'
  ...
  const now = input.now ?? Math.floor(Date.now() / 1000)
  // Allow a small issuer clock lead; never move an assertion's expiry forward.
