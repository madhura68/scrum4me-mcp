...
- **Publiceer `capabilities` op de worker-rij (codex plan-review P1-3):** geef in `run-one-job.ts` de `capabilities`-array mee aan `registerWorker` (:220) én `startHeartbeat` (:224) — beide ondersteunen het al (`worker.ts:11,29`, `heartbeat.ts:9,43`) en `const capabilities = readWorkerCapabilities()` staat al op :204. Anders registreert de worker met `capabilities: []` en blokkeert de workers-UI-gate (`capabilities has 'review'`, `manual-jobs.ts:200`) een codex-review-job ondanks claimbare agents. Additief, voor beide runtimes correct; de canary (§8) verifieert dat de live codex-worker-rij `review` adverteert.
...
| Twee promptbronnen (runner vs payload) raken uit sync | RESOLVED (§4): de docker-runner (`run-one-job.ts:364`) is gezaghebbend → `getKindPromptText(ctx.kind, runtime)`; `payload.prompt_text` alleen optioneel consistent/documentair. |
...
- **mac:codex — round-2: GO ✅** (msg b44fc4be). 5 round-1-fixes correct verwerkt; geen P1/P2. 2 niet-blokkerende P3-doc-nits (stale §10-promptbron-rij; verkeerd `model_id`-anker → `update-job-status.ts:853-855`) in dezelfde docs-pass opgeruimd. **GO for implementation.**
- **mac:codex — implementatieplan round-1: NO-GO → fixes verwerkt** (msg 9e311e45). 3 P1 (codex-prompt `update_idea_plan_md({ markdown })` niet `plan_md`; Task 6-test-mock `launch_preview_json.context.ideaId`; `capabilities` → `registerWorker`/`startHeartbeat` zodat de UI-gate de codex-worker telt), 1 P2 (host-forceren via pauze/semantische-tag, niet `required_capability=LOW_P`), 1 P3 (tsx-rooktest niet-bindend). Alle tegen de bron geverifieerd; plan + spec (§4/§8) bijgewerkt; round-2-confirm aangevraagd.
