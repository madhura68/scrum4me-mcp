...
2. **Prisma-modellen** `AgentMessage` / `AgentMessageArchive` in scrum4me-shared met `@@map("agent_message")` / `@@map("agent_message_archive")`: tabel- en kolomnamen identiek aan `s4m-queue/migrations/001_init.sql` + `002_archive.sql`, met **twee bewuste afwijkingen t.o.v. `001_init.sql`**: (a) de `source`-CHECK wordt uitgebreid met `'mcp'` (zie §5.1) — bestaande writers (`cli`, `dashboard`) blijven geldig — en (b) een extra index op `in_reply_to` (punt 3 hieronder). Check-constraints (`reply_link_matches_type`, type/status-CHECKs) en `agent_message_claim_idx (to_server, to_model, status, created_at)` gaan mee in de SQL-migratie.
...
**Validaties** (strenger dan de CLI): bericht bestaat (`QUEUE_NOT_FOUND`); status niet terminal (`QUEUE_ALREADY_TERMINAL`, atomair). **Claimer-check, tweetraps met vaste foutprecedentie.** Het lease-register is een map `message_id → claim_token`. Een aanroep zónder token op een `claimed` bericht krijgt direct `QUEUE_NOT_CLAIMER` (§7); de registerprecedentie hieronder geldt voor token-dragende aanroepen. Lookupvolgorde: (a) géén registry-entry voor `message_id` → `QUEUE_CLAIM_EXPIRED` (claim is niet van deze proces-incarnatie — ook als de rij nog `claimed` staat); (b) entry aanwezig maar het aangeleverde token ontbreekt of mismatcht → `QUEUE_NOT_CLAIMER`; (c) atomair in de DB — status is `claimed` én `claimed_by` is **exact** gelijk aan de verwachte volledige waarde (strikte gelijkheid, geen substring/`LIKE`), anders `QUEUE_NOT_CLAIMER` (vangt races met sweep en herclaim). Na terminale afronding of rollback wordt de entry uit het register verwijderd. Op een `pending` request blijft `queue_done` mét reply **tokenloos** toegestaan (de bestaande FIFO-bypass — per id antwoorden zonder eerst te claimen, zoals CLI `done <id> --reply`); wordt er echter wél een `claim_token` meegegeven op een `pending` bericht, dan signaleert dat een verlopen claim en volgt `QUEUE_CLAIM_EXPIRED` — een zombie-afronder mag niet stilletjes via de bypass binnenkomen.
...
**Ronde 6 — codex (2026-07-12): GO** op `4366d43..755ab59` (reviewdocument: `docs/superpowers/reviews/2026-07-12-s4m-queue-mcp-integration-round-6-review.md`). Twee minors, direct in de spec verwerkt:
