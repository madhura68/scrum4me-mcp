...
**Eigenaarscontract (transportbestendig):** de claim genereert een onvoorspelbaar token en schrijft `claimed_by = 'mcp:<instance_id>:<claim_token>'` — het bestaande text-veld, geen schema-uitbreiding (de CLI schrijft daar al `<server>:<pid>`). Het token bewijst eigenaarschap bij `queue_done`/`queue_fail`. De claim is gebonden aan de **proces-incarnatie** die hem uitgaf (§6.1): sterft of herstart het MCP-proces, dan verloopt de lease en requeue't de sweep de claim — er is bewust géén cross-process-voortzetting. (Een resume-contract zou van de agent vragen een MCP-herstart te detecteren, en dat kan hij niet; een verplichting die de client niet kan nakomen is geen contract.) Een done/fail mét token op een inmiddels gerequeued bericht krijgt `QUEUE_CLAIM_EXPIRED`, op een door een ander herclaimd bericht `QUEUE_NOT_CLAIMER`; in beide gevallen gooit de sessie lokaal werk weg en begint desgewenst opnieuw via `queue_next` — het bestaande `JOB_CANCELLED`-patroon uit deze repo. De `<instance_id>`-component in `claimed_by` is puur audit; de sweep kijkt er niet naar.
...
| Done/fail mét `claim_token` op een gerequeued (pending) bericht — verlopen claim | `QUEUE_CLAIM_EXPIRED`: lokaal werk weggooien, opnieuw `queue_next` (JOB_CANCELLED-patroon) |
