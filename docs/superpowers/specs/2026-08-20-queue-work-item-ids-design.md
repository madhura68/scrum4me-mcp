...
- `queue_push` overschrijft een eventueel door de caller meegegeven `meta.work_item`
  ...
  net als de rest van meta) — het MCP-pad met parameters is de geadviseerde route.
...
| Input | Afgeleid |
|---|---|
| `task_id` | `story_id`, `sprint_id` (kan ontbreken — `Task.sprint_id` is nullable), `product_id` |
...
- **Cold-store buiten scope:** `agent_message_archive` wordt niet doorzocht.
  `include_archived` betreft alleen gearchiveerde rijen in de actieve tabel.
...
| `__tests__/queue-work-item.test.ts` (nieuw) | unit: afleiding per input-vorm, consistentie-fouten, onbestaande ids (gemockte Prisma) |
| `__tests__/queue-find-by-work-item.test.ts` (nieuw, fase 2) | unit: AND-filter, reply-bijvoeging, archived-default, limiet/truncated |

Integratietests (echte Postgres) zijn optioneel; de bestaande
`queue-entity-transparency.test.ts` bewaakt al dat meta byte-exact doorkomt.

## 7. Wat dit oplevert

- Een agent tagt een queue-taak met `task_id` en elke andere agent vindt later met
  één tool-call de volledige berichtgeschiedenis van die taak, story of sprint —
...
- CLI-ondersteuning in `s4m-queue` (JP kan via `--meta-file` handmatig een
  `work_item`-blok zetten; dat pad valideert niet — bewust geaccepteerd).
