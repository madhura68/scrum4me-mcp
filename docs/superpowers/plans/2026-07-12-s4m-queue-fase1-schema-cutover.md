...
| Cleanup-bin prod | **Er is GEEN timer.** Vastgesteld 2026-07-16 door de server-agent: geen `s4m-queue-cleanup.timer`, geen service-unit, geen cron — op scrum4me-srv draait de cleanup nóóit. De eerdere planregel ("dagelijks 03:00") was fictie. De bin bestaat wél: `/usr/bin/s4m-queue-cleanup` → `/usr/lib/node_modules/s4m-queue` → `/home/janpeter/s4m-queue/dist/cleanup.js`. Checkout is **`/home/janpeter/s4m-queue`**, niet `~/Development/s4m-queue`. Server draait Node v22 terwijl de package ≥24 eist (EBADENGINE; build slaagt) |
| **Herkomst van de LIVE tabel** | **`ops_dashboard.agent_message` komt NIET uit `s4m-queue/migrations/001_init.sql`** — die is een *testfixture* die productie nooit raakte. De echte bron is `Ops-dashboard/prisma/migrations/20260528233000_add_agent_message` (Prisma, toegepast 2026-05-28). `ops_dashboard` is de Ops-dashboard-DB (FlowRun, Session, User, WorkerRun…) met `agent_message` als gast. Gevolg: onze DDL-equivalentie is bewezen tegen de **fixtures**, niet tegen productie |
| Live vs. onze nieuwe tabel | **Kolommen identiek** (17, zelfde volgorde/types/defaults) — daarom verhuist de data schoon. Verschillen, bewust geaccepteerd (besluit JP 2026-07-16): CHECK-namen `_chk` vs `_check`; FK `ON UPDATE CASCADE` vs `NO ACTION`; claim-index `agent_message_to_server_to_model_status_created_at_idx` vs `agent_message_claim_idx`. Alle drie cosmetisch: de CLI noemt nooit een constraint- of indexnaam, en `ON UPDATE` doet niets bij uuid-PK's die niet muteren. Onze source-CHECK is een superset (`+'mcp'`), dus alle bestaande rijen passen |
| **`agent_message_archive` bestaat NIET in productie** | `002_archive.sql` is óók fixture-only. Consistent met "geen timer": nooit gearchiveerd → tabel nooit nodig → niemand merkte het. De cutover verhuist dus **één** tabel (263 rijen op 2026-07-16), en het archief bestaat ná de cutover voor het eerst |
